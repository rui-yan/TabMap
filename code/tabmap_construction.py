...
class TabMapGenerator:
    ...
    def _create_feature_distance_matrix(self, data):
        """
        Computes the distance matrix among the features using the specified metric.

        Parameters:
        data (ndarray): Data matrix with shape (sample_size, num_features).

        Returns:
        ndarray: Pairwise distance matrix among features.
        """
        if self.metric == 'gower':
            return 1 - gower.gower_matrix(data.T)
        else:          
            return pairwise_distances(data.T, metric=self.metric)
    ...
    def fit(self, data, row_num=None, col_num=None, truncate=False):
        ...
        feat_dist_mat = self._create_feature_distance_matrix(data)
    ...
    def fit_transform(self, data, row_num=None, col_num=None, truncate=False):
        ...
        """
        self.fit(data, row_num, col_num, truncate)
