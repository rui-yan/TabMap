...
class Model_Evaluation:
    ...
    def model_predict(self, model, features: np.array):
        """
        Predict using the specified model and features.
        """
        if self.model_id in ML_MODELS:
            y_pred = model.predict(features)
            y_prob = model.predict_proba(features) if hasattr(model, 'predict_proba') else None
        
        elif self.model_id in DL_MODELS:
            testset = TabularDataset(features)
