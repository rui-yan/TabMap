...
def train_tabnet(config, data_dir, train_idx, valid_idx, model_id, model_path, opt_metric, device='cuda'):
    ...
    criterion = nn.CrossEntropyLoss() if labels.ndim ==1 else nn.BCEWithLogitsLoss()
    ...
    model.fit(X_train=features[train_idx], 
              y_train=labels[train_idx],
              eval_set=[(features[valid_idx], labels[valid_idx])],
