...
from tabmap.config import DL_MODELS, DL_MODELS_IMAGE_BASED, ML_MODELS, METRICS
from tabmap.dataloader.dataset import load_data
from tabmap.hparams_tuner.ml_models_tuner import MLModelTuner
...
def main(args):
    ...
    # Load the dataset
    X, y, _ = load_data(data_dir, scaler_name=args.scaler_name, preprocessed=False)
        ...
        for fold_id, (train_idx_all, test_idx) in enumerate(skf.split(X, y)):
            ...
            for model_id in model_list:
                ...
                if model_id in DL_MODELS_IMAGE_BASED:
                    ...
                    X_img = generate_images(args, model_id, X, train_idx_all, test_idx, 
                                             save_path=images_save_path)
                    images_dict[model_id] = X_img
                ...
                data_config = {
                    ...
                    "input_size": images_dict[model_id].shape[1:] \
                        if model_id in DL_MODELS_IMAGE_BASED else X.shape[1],
                }
                
                if model_id in DL_MODELS:
                    tuner = DLModelTuner(data_config, train_idx, valid_idx, 
                                         model_id, fold_id,
                                         args.results_path,
                    ...
                    tuner = MLModelTuner(data_config, train_idx, valid_idx, 
                                         model_id, fold_id, 
                                         args.results_path, 
                                         use_default_hparams=args.use_default_hparams,
                                         random_seed=args.seed)
                ...
                if model_id in DL_MODELS_IMAGE_BASED:
                    _, y_pred = model_eval.model_predict(tuner.final_model, 
                                                         X=images_dict[model_id][test_idx])
                else:
                    _, y_pred = model_eval.model_predict(tuner.final_model,
                                                         X=X[test_idx])
                
                predictions_test = pd.DataFrame({"model": model_id, "fold": fold_id, "trial": trial,  
                                                 "ground_truth": y[test_idx].ravel(), "pred": y_pred.ravel()})
